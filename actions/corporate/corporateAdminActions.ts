...
/* ─────────────────────────────────────────────
   APPROVE INQUIRY → CREATE ACCOUNT
   ───────────────────────────────────────────── */
...
export async function approveInquiryAndCreateAccount(
  ...
  try {
    ...
    if (inquiry.status !== "PENDING" && inquiry.status !== "CONTACTED") {
      return { ok: false, error: "Inquiry already processed." };
    }

    // Wrap everything in a transaction so if any step fails, the whole thing rolls back
    const { account, userId } = await db.$transaction(async (tx) => {
      ...
      if (user) {
        // Add CORPORATE role if not present
...
export async function resendCorporateWelcomeEmail(accountId: string) {
  ...
  try {
    ...
    const emailResult = await sendCorporateWelcomeEmail({
      to: contact.user.email ?? account.billingEmail,
