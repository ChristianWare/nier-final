...
import styles from "./AdminCalendarDriverDay.module.css";
...
export default async function AdminCalendarDriverDayPage(props: {
  params: Promise<{ driverId: string; ymd: string }>;
  searchParams: Promise<SearchParams>;
}) {
  const params = await props.params;
  const sp = await props.searchParams;
  const { driverId, ymd } = params;

  if (!ymd || !isValidYmd(ymd)) notFound();
  if (!driverId) notFound();
  ...
  const baseRoute = `/admin/calendar/drivers/${driverId}/${ymd}`;

  // Where clause — always scoped to this driver via assignment
  function buildWhere(opts: {
    ...
    if (opts.status === "ALL") {
      ...
      where.status = opts.status as BookingStatus;
    }

    if (opts.paid) where.payment = { is: { status: "PAID" } };

    return where;
  }

  const where = buildWhere({ status, paid });
  ...
  const statusCountsArr = await Promise.all(
    STATUSES.map(async (s) => {
      const c = await db.booking.count({
        where: buildWhere({ status: s, paid }),
      });
      return [s, c] as const;
    }),
  );
  ...
  return (
    <section
      ...
      aria-label={`Rides for ${driverName} on ${ymd}`}
    >
        ...
        <div className={styles.headerTop}>
          ...
          <div className={styles.driverNameplate}>
            {driver.image ? (
              ...
              <div className={styles.driverAvatarFallback}>
                {driverName
                  .split(" ")
            ...
            <div className={styles.driverNameplateInfo}>
              <span className={styles.driverNameplateLabel}>Driver</span>
              <span className={styles.driverNameplateName}>{driverName}</span>
            </div>
          </div>
          ...
          <div className={styles.headerActions}>
            <Link href='/admin/calendar' className='backBtn'>
              <Arrow className='backArrow' /> Back to Calendar
            </Link>
          </div>

          <div className={styles.meta}>
            <strong style={{ fontSize: "1.4rem" }}>{totalCount}</strong> ride
            {totalCount !== 1 ? "s" : ""} assigned
            {totalCount > 0 ? (
      ...
      {bookings.length === 0 ? (
        <div className={styles.empty}>
          ...
          <p className={styles.emptyCopy}>
            {driverName} has no rides matching these filters on this date.
          </p>
            ...
            <table className={styles.table}>
              ...
              <tbody>
                {bookings.map((b) => {
                  ...
                  const unitName = b.assignment?.vehicleUnit?.name ?? null;
                  const unitPlate = b.assignment?.vehicleUnit?.plate ?? null;
                  ...
                  return (
                    <tr key={b.id} className={styles.tr}>
                      ...
                      >
                        ...
                        {unitName ? (
                          <div className={styles.cellStack}>
                            <div className={styles.cellStrong}>{unitName}</div>
                            {unitPlate && (
                              <div className={styles.cellSub}>{unitPlate}</div>
                            )}
                          </div>
                        ) : (
                          <div className={styles.cellSub}>—</div>
                        )}
                      </td>
...
function StatusTabs({
  ...
  return (
    <div className={styles.tabRow}>
      {STATUSES.map((s) => {
        const href = buildHref(baseRoute, {
          ...current,
...
function ToggleChips({
  ...
  const paidHref = buildHref(baseRoute, {
    ...current,
...
function Pagination({
  ...
  const prevHref = buildHref(baseRoute, {
    ...current,
    page: page - 1 > 1 ? String(page - 1) : undefined,
  });
  const nextHref = buildHref(baseRoute, {
    ...current,
    page: String(page + 1),
  });
