...
import styles from "./VehiclePhotoModal.module.css";
...
// Helper to center a 16:9 aspect ratio crop (good for vehicle photos)
...
export default function VehiclePhotoModal({
  ...
  // 4:3 aspect ratio for vehicle photos
  ...
  const getCroppedImg = useCallback(async (): Promise<Blob | null> => {
    ...
    // Output 800x600 for 4:3 aspect ratio
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const croppedFile = new File([croppedBlob], "vehicle-photo.jpg", {
        type: "image/jpeg",
      });

      const formData = new FormData();
      formData.append("file", croppedFile);
      formData.append("vehicleUnitId", vehicleUnitId);

      const uploadRes = await fetch("/api/admin/upload/vehicle-photo", {
        method: "POST",
        body: formData,
      });
    ...
    <Modal isOpen={isOpen} onClose={handleClose}>
      <div className={styles.container}>
        <div className={styles.header}>
          <h2 className='h3'>Update Vehicle Photo</h2>
          <p className='miniNote'>
            {imageSrc
              ? "Drag the corners to adjust the crop area"
              : `Upload a photo for ${vehicleName || "this vehicle"}`}
          </p>
        </div>
