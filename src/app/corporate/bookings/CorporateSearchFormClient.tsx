...
export default function CorporateSearchFormClient({
  current,
  defaultValue,
}: {
  ...
  function apply(nextQ: string, action: PendingAction) {
    const q = nextQ.trim();
  ...
  return (
    <form className={styles.searchRow} onSubmit={onSubmit}>
      <input
        ...
        placeholder='Search passenger, address, service, confirmation #…'
        disabled={isPending}
