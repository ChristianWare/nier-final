...
const DENU = {
  address: "Denū Hotel & Spa, 1 E Adams St, Phoenix, AZ 85004",
...
// ─── Route constants ──────────────────────────────────────────────────────────
const SKY_HARBOR_ROUTE = { distanceMiles: 5, durationMinutes: 12 };
const MESA_GATEWAY_ROUTE = { distanceMiles: 31, durationMinutes: 40 };

const MAX_PASSENGERS_ONLINE = 14;

// ─── Types ────────────────────────────────────────────────────────────────────
type Direction =
  "to_denu_sky" | "from_denu_sky" | "to_denu_mesa" | "from_denu_mesa";

export type DenuVehicleDTO = {
  id: string;
  name: string;
...
export type DenuServiceTypeDTO = {
  id: string;
  name: string;
  slug: string;
...
function calcVehicleConfig(
  passengers: number,
  suvVehicle: DenuVehicleDTO,
  vanVehicle: DenuVehicleDTO,
...
function isMesaDirection(d: Direction | "") {
  return d === "to_denu_mesa" || d === "from_denu_mesa";
}

function isAirportPickupDirection(d: Direction | "") {
  return d === "to_denu_sky" || d === "to_denu_mesa";
}
...
function directionLabel(d: Direction) {
  switch (d) {
    case "to_denu_sky":
      return "Sky Harbor → Denū Hotel";
    case "from_denu_sky":
      return "Denū Hotel → Sky Harbor";
    case "to_denu_mesa":
      return "Mesa Gateway → Denū Hotel";
    case "from_denu_mesa":
      return "Denū Hotel → Mesa Gateway";
  }
}
...
// ─── Component ────────────────────────────────────────────────────────────────
export default function DenuBookingWizard({
  serviceType,
  suvVehicle,
  vanVehicle,
  mesaSuvVehicle,
  ...
  serviceType: DenuServiceTypeDTO;
  suvVehicle: DenuVehicleDTO;
  vanVehicle: DenuVehicleDTO;
  mesaSuvVehicle: DenuVehicleDTO;
  mesaVanVehicle: DenuVehicleDTO;
  ...
  const pickupLocation = isAirportPickup ? airportLocation : DENU;
  const dropoffLocation = isAirportPickup ? DENU : airportLocation;
    ...
    try {
      if (savedLegs.length > 0) {
        const currentLeg = {
          ...
          flightTerminal: v.flightTerminal || null,
          flightGate: null,
          eventType: "Hotel Transfer — Denū Hotel & Spa",
        };
        ...
        const allLegs = [
          ...savedLegs.map((sl) => {
            ...
            const slPickup = slIsAirportPickup ? slAirport : DENU;
            const slDropoff = slIsAirportPickup ? DENU : slAirport;
    ...
    () => [
      {
        ...
        step: 1,
        priority: "critical",
        sectionId: "denu-field-direction",
      },
      {
        key: "datetime",
        label: "Date & Time",
      ...
      },
      {
        ...
        step: 1,
        priority: "critical",
        sectionId: "denu-field-passengers",
      },
      {
        key: "vehicle",
        label: "Vehicle",
  ...
  return (
    <section id='denu-booking' className={styles.container}>
      <LayoutWrapper>
            ...
            <div className={styles.wizard}>
              ...
              {step === 1 && (
                <div className={`${styles.contentBox} ${styles.stepPane}`}>
                  ...
                  {/* Direction */}
                  <div id='denu-field-direction' className={styles.sectionBox}>
                    ...
                    >
                      ...
                      <optgroup label='Phoenix Sky Harbor Airport'>
                        <option value='to_denu_sky'>
                          Sky Harbor → Denū Hotel
                        </option>
                        <option value='from_denu_sky'>
                          Denū Hotel → Sky Harbor
                        </option>
                      </optgroup>
                      <optgroup label='Mesa Gateway Airport'>
                        <option value='to_denu_mesa'>
                          Mesa Gateway → Denū Hotel
                        </option>
                        <option value='from_denu_mesa'>
                          Denū Hotel → Mesa Gateway
                        </option>
                      </optgroup>
                    </select>
                    {direction && (
                      <p className='miniNote'>
                        {isAirportPickup
                          ? `Pickup at ${isMesaDirection(direction) ? "Mesa Gateway" : "Sky Harbor"} — drop-off at Denū Hotel & Spa.`
                          : `Pickup at Denū Hotel & Spa — drop-off at ${isMesaDirection(direction) ? "Mesa Gateway" : "Sky Harbor"}.`}
                      </p>
                    )}
                  </div>

                  {/* Date & time */}
                  <div id='denu-field-datetime' className={styles.sectionBox}>
                    <label
                      className={labelCx(
                  ...
                  {/* Passengers & Luggage */}
                  <div id='denu-field-passengers' className={styles.sectionBox}>
                    <div className={styles.fieldRow}>
                      ...
                      <div style={{ display: "grid", gap: 10 }}>
                        ...
                        <p className='miniNote'>
                          Include all checked bags in your count
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              {step === 2 && vehicleConfig && (
                <div className={`${styles.contentBox} ${styles.stepPane}`}>
                  ...
                  <div id='denu-field-vehicle' className={styles.vehicleCard}>
              ...
              {step === 3 && (
                ...
                >
                  ...
                  {!isAuthed ? (
                    <div id='denu-field-contact' className={styles.sectionBox}>
                      <div style={{ display: "grid", gap: 10 }}>
